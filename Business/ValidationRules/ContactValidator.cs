...
namespace Business.ValidationRules
{
    public class ContactValidator : AbstractValidator<Contact>
    {
        public ContactValidator()
        {
            ...
            RuleFor(x => x.ContactUserEmail).NotEmpty().WithMessage(Messages.EmailNotEmpty);
            RuleFor(x => x.ContactUserEmail).MaximumLength(50).WithMessage(Messages.EmailMaximumLength);
            RuleFor(x => x.ContactUserEmail).EmailAddress().WithMessage(Messages.InvalidEmailAddress);
